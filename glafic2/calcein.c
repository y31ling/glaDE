...
double calcein_king(int i)
{
  ...
  king_set_xt(para_lens[i][7]);
  b_sav = b_func_king(para_lens[i][1], para_lens[i][6]);
  x = para_lens[i][6];

  if((calcein_king_func(smallcore) > 0.0) && (calcein_king_func(XMAX_CALCEIN) < 0.0)){
    r = gsl_zbrent(calcein_king_func, smallcore, XMAX_CALCEIN, TOL_ZBRENT_CALCEIN) * x;
  } else {
    r = CALCEIN_NAN;
  }
