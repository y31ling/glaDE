...
static double xt_king_sav;   /* dimensionless tidal radius = 10^c */
static double f0_king_sav;   /* = 1/sqrt(1 + xt^2) */
static double norm_king_sav; /* normalization N */
...
void lensmodel_get_i(int i, double tx, double ty, double *kap, double *gam1, double *gam2, double *phi, double *ax, double *ay, int alponly)
{
  ...
  *ay   = 0.0;
  ...
  case 26:
    kapgam_king(tx, ty, cx, cy, para_lens[i][1], para_lens[i][6], para_lens[i][7], para_lens[i][4], para_lens[i][5], kap, gam1, gam2, phi, ax, ay, alponly);
    break;
  }
...
/*--------------------------------------------------------------
  ...
  Surface density:
    Sigma(R) = Sigma0 * [1/sqrt(1+(R/rc)^2) - 1/sqrt(1+(rt/rc)^2)]^2
               for R <= rt, and 0 for R > rt

  Parameters:
    m  : total projected mass [M_sun]
    rc : core radius [arcsec]
    c  : concentration = log10(rt/rc)
    e  : ellipticity
    pa : position angle [deg]

  Normalization: N = ln(st) - 3/2 + 2*f0 - f0^2/2
    where st = sqrt(1+xt^2), f0 = 1/st, xt = 10^c

  dphi (projected deflection angle kernel):
    for x <= xt:
      dphi_king_dl(x) = 2*I(x)/(N*x)
      I(x) = ln(1+x^2)/2 - 2*f0*sqrt(1+x^2) + 2*f0 + f0^2*x^2/2
    for x > xt:
      dphi_king_dl(x) = 2/x  (point-mass limit)
*/

void king_set_xt(double c)
{
  double st;

  xt_king_sav = pow(10.0, c);
  st = sqrt(1.0 + xt_king_sav * xt_king_sav);
  f0_king_sav = 1.0 / st;
  norm_king_sav = log(st) - 1.5 + 2.0 * f0_king_sav - 0.5 * f0_king_sav * f0_king_sav;
  if(norm_king_sav < OFFSET_LOG) norm_king_sav = OFFSET_LOG;

  return;
}

double b_func_king(double m, double rc)
{
  double rr;
...
double kappa_king_dl(double x)
{
  double f;

  if(x >= xt_king_sav) return 0.0;

  f = 1.0 / sqrt(1.0 + x * x) - f0_king_sav;
  if(f <= 0.0) return 0.0;

  return f * f / norm_king_sav;
}
...
double dkappa_king_dl(double x)
{
  ...
  s2 = 1.0 + x * x;
  s3 = s2 * sqrt(s2);
  f = 1.0 / sqrt(s2) - f0_king_sav;
  if(f <= 0.0) return 0.0;

  return (-2.0) * x * f / (s3 * norm_king_sav);
}
...
double dphi_king_dl(double x)
{
  ...
  sx = sqrt(1.0 + x * x);
  I_king = 0.5 * log(1.0 + x * x)
           - 2.0 * f0_king_sav * sx
           + 2.0 * f0_king_sav
           + 0.5 * f0_king_sav * f0_king_sav * x * x;

  return 2.0 * I_king / (norm_king_sav * x);
}
...
void kapgam_king(double tx, double ty, double tx0, double ty0, double m, double rc, double c, double e, double pa, double *kap, double *gam1, double *gam2, double *phi, double *ax, double *ay, int alponly)
{
  ...
  checkmodelpar_mineq(e, 0.0);
  checkmodelpar_max(e, 1.0);
  checkmodelpar_min(rc, 0.0);
  checkmodelpar_mineq(c, 0.0);
  ...
  j1 = ell_integ_j(kappa_king_dl, 1);
  bpx = q * bx * j1;
  j0 = ell_integ_j(kappa_king_dl, 0);
  bpy = q * by * j0;
  ...
  *ay = bb * tt * py;
  ...
  if(alponly != 1){
    bpxx = 2.0 * q * bx * bx * ell_integ_k(dkappa_king_dl, 2) + q * j1;
    bpyy = 2.0 * q * by * by * ell_integ_k(dkappa_king_dl, 0) + q * j0;
    bpxy = 2.0 * q * bx * by * ell_integ_k(dkappa_king_dl, 1);
    ...
    *gam1 = 0.5 * bb * (pxx - pyy);
    *gam2 = bb * pxy;
    if(alponly < 0) *phi = 0.5 * q * bb * ell_integ_i(dphi_king_dl) * tt * tt;
  }
