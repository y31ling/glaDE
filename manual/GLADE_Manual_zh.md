...
7. [拟合引擎与算法](#7-拟合引擎与算法)
   — DE · 损失函数 · 后端与 GPU 批量 · MCMC · 延展源 · 独立验证
8. [读懂运行输出](#8-读懂运行输出)
...
- 每个对话框都有**文本编辑模式**（`切换文本编辑器`）：把整个对象写成一行
  ...
  分配到各字段。
- ⚠️ `pow`、`pert` 对话框的标签有误：引擎始终把对话框第一个"z"字段当作
  **透镜红移**、把 p1 字段当作基准源红移 `zs_fid`（与 §6.4 的引擎约定一致）——
  但这两个对话框的标签恰好标反了（第一个字段标着"参考红移/Fiducial zs"、p1
  标着"透镜面 z"）。请按引擎约定填数，不要按标签。（`gaupot` 的 p1 字段是速度
  弥散。）另外：红移填 `0` 会被静默换成默认值（透镜 0.5、源 2），非数字输入
  会变成 0。
