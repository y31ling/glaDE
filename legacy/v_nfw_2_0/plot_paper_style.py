...
def plot_paper_style_nfw(
    ...
    # 标注NFW sub-halo位置
    if nfw_params is not None and len(nfw_params) > 0:
        for i, (x_nfw, y_nfw, m_nfw, c_nfw) in enumerate(nfw_params, start=1):
            # 绘制NFW halo位置
            ax.scatter(x_nfw, y_nfw, marker='D', s=150, 
                      color='purple', edgecolor='black', linewidth=2, 
                      zorder=10, alpha=0.9)
            # 标注质量和浓度
            nfw_label = f"N{i}:\n{m_nfw:.1e}\nc={c_nfw:.1f}"
            ax.text(x_nfw, y_nfw - 0.028, nfw_label, 
        ...
        ax.scatter([], [], marker='D', s=150, color='purple', 
                  edgecolor='black', linewidth=2, label='NFW Sub-halo')
...
def plot_paper_style_nfw_compare(
    ...
    if subhalo_positions is not None and len(subhalo_positions) > 0:
        for i, params in enumerate(subhalo_positions, start=1):
            if len(params) == 4:
                x_sub, y_sub, m_sub, c_sub = params
                sub_label = f"N{i}:\n{m_sub:.1e}\nc={c_sub:.1f}"
            elif len(params) == 3:
                x_sub, y_sub, m_sub = params
                sub_label = f"S{i}: {m_sub:.1e}"
            else:
                continue
...
if __name__ == "__main__":
    ...
    print("支持 Point Mass 模型 (plot_paper_style) 和 NFW 模型 (plot_paper_style_nfw)")
