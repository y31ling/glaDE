...
obs_positions_mas = np.array([     # [mas]
    [-266.035, +0.427],
    [+118.835, -221.927],
    [+238.324, +227.270],
    [-126.157, +319.719],
...
obs_positions = np.zeros_like(obs_positions_mas)  # [arcsec]
obs_positions[:, 0] = -obs_positions_mas[:, 0] / 1000.0
obs_positions[:, 1] = obs_positions_mas[:, 1] / 1000.0

obs_magnifications = np.array([-35.6, 15.7, -7.5, 9.1])  # [dimensionless]
obs_mag_errors = np.array([2.1, 1.3, 1.0, 1.1])          # [dimensionless]
obs_pos_sigma_mas = np.array([0.41, 0.86, 2.23, 3.11])   # [mas]
...
# 使用智能查找 glafic 可执行文件
GLAFIC_BIN = find_glafic_bin("/home/luukiaun/glafic251018/glafic2/glafic")
