#!/usr/bin/env python3
"""
Version Point Mass Extend: Flexible Sub-halos Search + 有限尺寸 (tophat) 源
================================================================================
基于 v_pointmass_1_0，唯一的物理改动：把背景超新星从「点源」改成「有限半径的
tophat（均匀亮度圆盘）源」，并把源半径 source_radius 作为一个新的可调参数加入优化。
其余所有配置与 v_pointmass_1_0 完全一致、保持不变。

为什么用「源面采样平均」而不是 glafic 的 extend 像素成像
--------------------------------------------------------------------------------
预估超新星光球半径 ~ 5e-5 ~ 5e-4 mas (= 5e-8 ~ 5e-7 arcsec)，比 glafic 的成像
像元 pix_ext=0.01 arcsec (=10 mas) 小约 5 个数量级。任何可行的像素网格都无法分辨
如此小的源，所以 glafic 自带的 `extend tophat` + writeimage 成像在这里完全不适用。

物理上正确、且唯一可行的 CPU 做法是「源面平均」：
    扩展源某一宏观像的流量放大率 = 在源圆盘上对点源放大率 |mu_point| 的面积平均。
对 tophat（均匀亮度）即：
    mu_ext,i = sign_i * < sum_micro |mu| >_(disk)      （sign_i 为该宏观像的宇称）
当半径 r -> 0 时严格退化为原 v_pointmass 的点源结果，因此与原版连续衔接。
近微焦散线（subhalo 点质量造成）处，有限尺寸会把发散的放大率抹平，这正是有限源
真正起作用、也是本版本想要探究的区域。

新增/改动一览（其余与 v_pointmass_1_0 相同）：
1. SOURCE_PROFILE / SOURCE_RADIUS_*_MAS / SOURCE_NSAMPLE 配置
2. source_radius 作为优化向量的最后一维（可调参数）
3. compute_model 用 tophat 源面采样平均计算每个宏观像的有限源放大率
"""
...
print("=" * 70)
print("Version Point Mass Extend: Sub-halos Search + 有限尺寸 (tophat) 源")
print("=" * 70)
...
BASELINE_LENS_DIR = '/home/luukiaun/glafic251018/work/glade/legacy/v_pointmass_extend/bestfit_default'
...
# ==================== 4.2 有限尺寸源 (tophat) 配置 [本版本核心] ====================
# 把超新星从「点源」改为「有限半径的 tophat 均匀圆盘」，并将半径作为可调参数优化。
SOURCE_PROFILE = 'tophat'        # 暂时只支持 'tophat'（均匀亮度圆盘）

# 源半径搜索范围（单位 mas）。预估膨胀到顶峰的超新星光球角半径 ~ 5e-5 ~ 5e-4 mas。
SOURCE_RADIUS_MAS_MIN = 5e-5
SOURCE_RADIUS_MAS_MAX = 5e-4

# 源圆盘的面积采样点数：扩展源放大率 = 在圆盘上对点源放大率做面积平均。
# 采样点越多越精确，但每次模型评估的 glafic 调用次数 ≈ (SOURCE_NSAMPLE + 1)，
# 计算量随之线性增加。tophat 在远离微焦散线处变化极缓，32~64 已足够；近焦散线
# 需要更多点才能解析边缘。设为 0 或 1 则退化为点源（等价于 v_pointmass）。
SOURCE_NSAMPLE = 32

# 内部使用的半径单位换算：优化时统一用 arcsec
SOURCE_RADIUS_ARCSEC_MIN = SOURCE_RADIUS_MAS_MIN / 1000.0
SOURCE_RADIUS_ARCSEC_MAX = SOURCE_RADIUS_MAS_MAX / 1000.0
# 基准模型 / 绘图 / 命令行验证时使用的代表性半径（取范围几何中点）
SOURCE_RADIUS_ARCSEC_DEFAULT = float(np.sqrt(SOURCE_RADIUS_ARCSEC_MIN * SOURCE_RADIUS_ARCSEC_MAX))
...
# [本版本] 源半径恒为可调参数，占优化向量的最后一维
n_params_radius = 1

n_params_extra = n_params_source + n_params_lens + n_params_radius
...
print(f"\n有限尺寸源 (tophat) 配置 [本版本核心]:")
print(f"  source_profile = {SOURCE_PROFILE}")
print(f"  源半径搜索范围: [{SOURCE_RADIUS_MAS_MIN:.2e}, {SOURCE_RADIUS_MAS_MAX:.2e}] mas")
print(f"                  = [{SOURCE_RADIUS_ARCSEC_MIN:.2e}, {SOURCE_RADIUS_ARCSEC_MAX:.2e}] arcsec")
print(f"  圆盘面积采样点数 SOURCE_NSAMPLE = {SOURCE_NSAMPLE}")
print(f"  (每次模型评估的 glafic 调用次数 ≈ {SOURCE_NSAMPLE + 1})")
...
# ==================== 有限尺寸 (tophat) 源：圆盘面积采样 ====================
def tophat_disk_samples(cx, cy, radius, n):
    """
    在以 (cx, cy) 为中心、半径为 radius 的圆盘内做「等面积」采样（向日葵/Fibonacci
    螺旋点阵），用于对 tophat（均匀亮度）源做面积平均。

    返回 shape=(m, 2) 的采样点坐标数组：
      - radius<=0 或 n<=1 时返回单个中心点（退化为点源）；
      - 否则返回 n 个均匀覆盖圆盘的点（每点等权 = tophat 均匀亮度）。
    """
    if radius is None or radius <= 0.0 or n <= 1:
        return np.array([[cx, cy]])
    k = np.arange(n)
    # r_k ∝ sqrt(k) 保证等面积；中心偏移 0.5 让最内点不落在圆心
    r = radius * np.sqrt((k + 0.5) / n)
    golden = np.pi * (3.0 - np.sqrt(5.0))   # 黄金角
    theta = k * golden
...
def _solve_drop_central(sx, sy):
    """
    在源面点 (sx, sy) 解透镜方程，返回 [(x, y, mag), ...]。
    与原版一致：若得到 5 个像（anfw 等的多余中心像），剔除 |μ| 最小的那个。
    返回 None 表示该采样点解像异常（像数 < 4），调用方可跳过。
    """
    res = glafic.point_solve(source_z, sx, sy, verb=0)
    n_images = len(res)
    if n_images == 5:
        abs_mags = [abs(d[2]) for d in res]
        drop_idx = int(np.argmin(abs_mags))
        res = [d for i, d in enumerate(res) if i != drop_idx]
    elif n_images < 4:
        return None
    return [(d[0], d[1], d[2]) for d in res]


# ==================== 计算模型函数 ====================
def compute_model(subhalo_params_list, verbose=False, src_x=None, src_y=None,
                  lens_params_dict=None, source_radius=None):
    """
    计算多 sub-halo 模型（有限尺寸 tophat 源版本）。

    subhalo_params_list: [(x, y, mass), ...]
    src_x, src_y:   可选的源中心位置（None 用全局默认值）
    lens_params_dict: 可选的透镜参数字典（None 用全局默认值）
    source_radius:  tophat 源半径 [arcsec]。None -> 用 SOURCE_RADIUS_ARCSEC_DEFAULT；
                    <=0 -> 退化为点源（与 v_pointmass 完全一致）。

    有限源放大率算法（源面平均）：
      1) 在源中心解像，确定 4 个宏观像的参考位置与宇称 sign_i；
      2) 在 tophat 圆盘上等面积采样，对每个采样点解像，把每个像按最近邻归入
         对应宏观像，累加 |μ|（流量）与「以 |μ| 加权的位置」；
      3) 对所有有效采样点取平均：mu_ext,i = sign_i * <Σ|μ|>，pos_ext,i = 流量加权质心。
    """
    # 使用传入的参数或默认值
    ...
    use_radius = source_radius if source_radius is not None else SOURCE_RADIUS_ARCSEC_DEFAULT
    ...
    # ---- (1) 源中心解像：确定 4 个宏观参考像（位置 + 宇称） ----
    central = _solve_drop_central(use_src_x, use_src_y)
    if central is None or len(central) != 4:
        if verbose:
            n_c = 0 if central is None else len(central)
            print(f"  Warning: central source produced {n_c} images (expected 4)")
        glafic.quit()
        return None, None, None, 1e10

    ref_pos = np.array([[d[0], d[1]] for d in central])      # (4,2) 宏观参考位置
    ref_sign = np.array([np.sign(d[2]) for d in central])    # (4,) 宏观像宇称
    ref_sign[ref_sign == 0] = 1.0

    # ---- (2) tophat 圆盘等面积采样，逐点解像并归入宏观像 ----
    samples = tophat_disk_samples(use_src_x, use_src_y, use_radius, SOURCE_NSAMPLE)

    flux_acc = np.zeros(4)            # Σ_samples Σ_micro |μ|
    pos_acc = np.zeros((4, 2))        # Σ_samples (Σ_micro |μ| * pos)
    n_valid = 0
    for (sx, sy) in samples:
        imgs = _solve_drop_central(sx, sy)
        if imgs is None:
            continue
        n_valid += 1
        for (ix, iy, imag) in imgs:
            # 把该像按最近邻归入某个宏观像（微像总是紧贴其所属宏观像）
            d2 = (ref_pos[:, 0] - ix) ** 2 + (ref_pos[:, 1] - iy) ** 2
            j = int(np.argmin(d2))
            w = abs(imag)
            flux_acc[j] += w
            pos_acc[j, 0] += w * ix
    ...
    if n_valid == 0:
        if verbose:
            print(f"  Warning: no valid disk samples")
        return None, None, None, 1e10

    # ---- (3) 平均 → 有限源放大率（带宇称符号）与流量加权位置 ----
    pred_magnifications = ref_sign * (flux_acc / n_valid)
    pred_positions = np.zeros((4, 2))
    for j in range(4):
        if flux_acc[j] > 0:
            pred_positions[j] = pos_acc[j] / flux_acc[j]
        else:
            pred_positions[j] = ref_pos[j]   # 兜底：用中心参考位置
...
# [本版本] 最后追加源半径 bounds（arcsec），与 objective_function 的解析顺序一致
bounds.append((SOURCE_RADIUS_ARCSEC_MIN, SOURCE_RADIUS_ARCSEC_MAX))

print(f"\n搜索参数空间（{n_params}维）:")
print(f"  Source radius (参数索引 {n_params-1}, 最后一维):")
print(f"    r=[{SOURCE_RADIUS_MAS_MIN:.2e}, {SOURCE_RADIUS_MAS_MAX:.2e}] mas"
      f" = [{SOURCE_RADIUS_ARCSEC_MIN:.2e}, {SOURCE_RADIUS_ARCSEC_MAX:.2e}] arcsec")
...
# [本版本] 解析最优源半径（最后一维），单位 arcsec
best_source_radius = result[idx]
idx += 1
...
best_pos, best_mag, best_delta_pos, best_mag_chi2 = compute_model(
    ...
    source_radius=best_source_radius
)
...
# [本版本] 最优源半径
print(f"\n最优有限源 (tophat) 半径:")
print(f"  source_radius = {best_source_radius*1000:.4e} mas"
      f" = {best_source_radius:.4e} arcsec")
print(f"  搜索范围: [{SOURCE_RADIUS_MAS_MIN:.2e}, {SOURCE_RADIUS_MAS_MAX:.2e}] mas")
_r_lo_frac = (best_source_radius - SOURCE_RADIUS_ARCSEC_MIN) / (SOURCE_RADIUS_ARCSEC_MAX - SOURCE_RADIUS_ARCSEC_MIN)
if _r_lo_frac < 0.02 or _r_lo_frac > 0.98:
    print(f"  ⚠ 注意：最优半径贴近搜索边界（位于范围 {_r_lo_frac*100:.1f}% 处），"
          f"可能说明半径不受数据约束（似然对半径近似平坦）或边界需放宽。")
...
if MCMC_ENABLED:
    ...
    def log_probability(params):
        ...
        # [本版本] 源半径（最后一维）。自定义先验时需显式检查物理边界
        src_radius_opt = params[idx]
        idx += 1
        if MCMC_CUSTOM_RANGE:
            if not (SOURCE_RADIUS_ARCSEC_MIN <= src_radius_opt <= SOURCE_RADIUS_ARCSEC_MAX):
                return -np.inf
    ...
    param_names.append('src_radius_arcsec')   # [本版本] 最后一维：源半径
...
with open(params_file, 'w') as f:
    ...
    # [本版本] 有限源 (tophat) 半径
    f.write(f"# Finite source (tophat) radius\n")
    f.write(f"source_profile = {SOURCE_PROFILE}\n")
    f.write(f"source_nsample = {SOURCE_NSAMPLE}\n")
    f.write(f"source_radius_arcsec = {best_source_radius:.10e}  # arcsec\n")
    f.write(f"source_radius_mas = {best_source_radius*1000:.10e}  # mas\n\n")
...
# [本版本] glafic 命令行的 findimg 给的是「点源」放大率，无法直接和 Python 的
# 「有限源 (tophat)」放大率 best_mag 对比。为做同口径的一致性核验，这里用
# source_radius=0（即点源）重算一份 Python 参考结果 ver_pos / ver_mag，与 CLI 对齐。
# 有限源放大率 best_mag 仍用于科学图与参数文件。
print("\n[验证口径] CLI findimg = 点源；用 radius=0 重算 Python 点源参考做一致性核验。")
ver_pos, ver_mag, _, _ = compute_model(
    best_params, src_x=best_source_x, src_y=best_source_y,
    lens_params_dict=best_lens_params if lens_modify else None,
    source_radius=0.0
)
if ver_pos is None:
    print("  警告: 点源参考重算失败，回退到有限源结果（验证可能出现预期内差异）")
    ver_pos, ver_mag = best_pos, best_mag
...
with open(verify_input_file, 'w') as f:
    f.write("# ========================================\n")
    f.write("# Version Point Mass Extend 验证文件\n")
    f.write("# ========================================\n")
    ...
    f.write(f"# 有限源 (tophat) 最优半径: {best_source_radius*1000:.4e} mas"
            f" = {best_source_radius:.4e} arcsec\n")
    f.write("#   注意：CLI findimg 只能解点源；下方 point 行即点源，用于位置/点源放大率核验。\n")
    f.write("#   有限源放大率（源面平均）由 Python 计算，无法用 CLI 在此尺度下复现。\n")
    f.write("# ========================================\n\n")
    ...
    try:
        ...
        if len(data.shape) == 1:
            ...
            if n_images_glafic in (4, 5):
                ...
                max_pos_diff = 0.0
                for i in range(4):
                    py_x = ver_pos[i, 0] * 1000  # 转换为 mas（点源参考）
                    py_y = ver_pos[i, 1] * 1000
                ...
                print(f"\n  {'Img':<5} {'Python μ(点源)':<15} {'glafic μ':<15} {'Δμ':<12} {'Δμ [%]':<12}")
                print(f"  {'-'*65}")
                ...
                with open(verify_report_file, 'w') as f:
                    ...
                    f.write("# 说明：CLI=点源放大率；Python 列为 radius=0 点源参考（与 CLI 同口径）。\n")
                    f.write("# 有限源 (tophat) 放大率见 best_params 文件与科学图。\n\n")
                    f.write("位置对比 (mas):\n")
                    f.write(f"{'Img':<5} {'Python x':<15} {'glafic x':<15} {'Δx':<12} "
                            f"{'Python y':<15} {'glafic y':<15} {'Δy':<12}\n")
                    f.write("-" * 95 + "\n")
                    for i in range(4):
                        py_x = ver_pos[i, 0] * 1000
                        py_y = ver_pos[i, 1] * 1000
                        gl_x = glafic_pos_matched[i, 0] * 1000
                        gl_y = glafic_pos_matched[i, 1] * 1000
                        diff_x = abs(py_x - gl_x)
                    ...
                    f.write("放大率对比 (Python 点源参考 vs CLI 点源):\n")
