...
def _solve_for_subhalos(subhalos):
    """subhalos: list of (x, y, mass, rc, c)."""
    if subhalos:
        ...
        rc_t = torch.tensor([[1e-3]], device=device, dtype=dtype)
        ck_t = torch.tensor([[1.0]], device=device, dtype=dtype)
...
def main():
    ...
    for i in range(n_active_subhalos):
        ...
        test[i * 5 + 2] = rng.uniform(cfg["logM_min"], cfg["logM_max"], pop)
        test[i * 5 + 3] = rng.uniform(cfg["rc_min"], cfg["rc_max"], pop)
        test[i * 5 + 4] = rng.uniform(cfg["c_min"], cfg["c_max"], pop)
    if device.type == "cuda":
        torch.cuda.synchronize()
    t0 = time.perf_counter()
    ...
    for img_idx in active_subhalos:
        cfg = subhalo_configs[img_idx]
        fixed_mass_range.extend([cfg["logM_min"], cfg["logM_max"]])
    ...
    # ---- MCMC -------------------------------------------------------------
    posterior_indices = [(active_subhalos[i], i * 5 + 2)
                         for i in range(n_active_subhalos)]
    param_names = []
    corner_labels = []
    for img_idx in active_subhalos:
    ...
    for i, (x, y, m, rc, c) in enumerate(subhalos):
        ...
        # king layout: (idx, model, z, M, x, y, e, pa, rc, c)
        extra_lens_lines.append(
            (idx, "king", lens_z, m, x, y, 0.0, 0.0, rc, c))
    ...
    sub_xy_mass = [(s[0], s[1], s[2]) for s in subhalos]
    gp.write_result_triptych(
        triptych_path,
        suptitle=f"iPTF16geu: {n_active_subhalos} King Sub-halos (GPU)",
    ...
    if COMPARE_GRAPH and n_active_subhalos > 0:
        ...
        gp.write_compare_triptych(
            compare_path,
            suptitle=f"iPTF16geu: Baseline vs {n_active_subhalos} King Sub-halos (GPU)",
            obs_positions=obs_positions, pred_positions=best_pos,
