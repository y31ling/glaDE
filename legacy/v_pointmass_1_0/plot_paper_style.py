...
def plot_paper_style(
    ...
    subhalo_positions=None,  # New: list of (x, y, mass) tuples
    show_2sigma=False  # New: 是否显示2σ横线
):
    """
    生成论文风格的三联图
    ...
    plt.tight_layout(rect=[0, 0.12, 1, 0.95])  # 为底部图例留更多空间
    plt.savefig(output_file, dpi=220, bbox_inches='tight')
    print(f"✓ 保存图片: {output_file}")
    plt.close()

def plot_paper_style_compare(
    img_numbers,
...
if __name__ == "__main__":
    ...
    print("支持 Point Mass 模型 (plot_paper_style)")
    print("支持比较模式 (plot_paper_style_compare)")
